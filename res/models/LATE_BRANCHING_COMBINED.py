...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.avgpools = nn.AdaptiveAvgPool2d((1, 1))
    ...
    def get_activations(self, x, layer_name):
        ...
        if layer_name == 'pre_fc':
            activations_dict[layer_name] = x
            return activations_dict

        x_out_1 = self.fc_1(x)
        relu_1 = nn.ReLU(inplace=True)
        x_out_1 = relu_1(x_out_1)
        
        x_out_2 = self.fc_2(x)
        relu_2 = nn.ReLU(inplace=True)
        x_out_2 = relu_2(x_out_2)
        
        if layer_name == 'fc':
            activations_dict['%s_rotation'%layer_name] = x_out_1
            activations_dict['%s_model'%layer_name] = x_out_2
            return activations_dict
